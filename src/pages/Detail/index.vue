        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 这里在加入购物车、进行路由跳转之前要发一次请求，把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    //表单元素修改产品个数(用户有可能输入任何值，所以要进行校验)
    changeSkuNum(e) {
      //将用户输入进来的文本 * 1，若不是数字则返回NaN
      let value = e.target.value * 1;
      //如果用户输入进来的数据非法（出现NaN或者小于1）
      if (isNaN(value) || value < 1) {
        this.skuNum = 1;
      } else {
        //正常大于1，但注意必须是整数不能出现小数
        this.skuNum = parseInt(value); //有可能会输入小数，所以要进行取整
      }
    },
    //加入购物车的回调函数
    async addShopcar() {
      //1:发请求---将产品加入到数据库（通知服务器）
      //在点击加入购物车这个按钮时，做的第一件事就是将参数带给服务器（发请求），通知服务器加入购物车的产品是谁
      try {
        //成功
        await this.$store.dispatch("addOrUpdateShopCart", {
          //这行代码的实质是：调用vuex仓库中的addOrUpdateShopCart函数（该函数返回一个Promise）
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        //进行路由跳转
        //在路由跳转的时候还需要将产品的信息带给下一级的路由组件
        //传递参数时带query参数，但不要带skuInfo这个对象，不然地址栏会很难看
        //一些简单的数据skuNum，通过query参数形式给路由组件传递过去
        //产品信息的数据（比如skuInfo，比较复杂），通过sessionStorage会话存储（不持久化，会话结束数据再消失）
        //无论是本地存储还是会话存储，一般存储的是字符串
